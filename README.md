...
## 🚀 Key Features
- Intelligent legal Document Analysis using Google Gemini 2.5 Pro (with Function Calling & FileSearch API).
...
| File handling | multer (for uploads), server integrates File Search API sending the document to Google Cloud RAG for precise chunking and return of all the relevant data  |
...
5. Result Delivery: the server sends final JSON, Markdown and metadata via SSE; the client parses and displays results, allows file download.
...
- Inteligentna analiza pravnih dokumenata koristeći Google Gemini 2.5 Pro (Function Calling & File Search API).
...
|---|---|
| AI model | Google Gemini 2.5 Pro (pozivanje funkcija i pretraga datoteka) |
| Frontend | React, TypeScript, Vite, Tailwind CSS |
| Bekend | Node.js, Express, TypeScript (tsx za brzo ponovno učitavanje) |
| Komunikacija | REST API, događaji poslati sa servera (SSE) |
| Rukovanje datotekama | multer (za otpremanje), server integriše File Search API šaljući dokument Google Cloud RAG-u za precizno grupisanje i vraćanje svih relevantnih podataka |
...
5. Prikaz rezultata: server šalje finalni JSON, Markdown i metapodatke kroz SSE; klijent parsira i prikazuje rezultate, omogućavajući preuzimanje fajlova.
...
## Autor
Razvijeno od strane Milana Putnika.
